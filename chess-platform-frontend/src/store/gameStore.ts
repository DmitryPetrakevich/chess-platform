...
export const useGameStore = defineStore("game", () => {
  ...
  function isSquareAttacked(square, byColor, board = pieces.value) {
    ...
    // Пешка
    if (byColor === "w") {
      if (tFile - 1 >= 0 && board[`${files[tFile - 1]}${tRank - 1}`] === "wP")
        return true;
      if (tFile + 1 <= 7 && board[`${files[tFile + 1]}${tRank - 1}`] === "wP")
        return true;
    } else {
      if (tFile - 1 >= 0 && board[`${files[tFile - 1]}${tRank + 1}`] === "bP")
        return true;
      if (tFile + 1 <= 7 && board[`${files[tFile + 1]}${tRank + 1}`] === "bP")
        return true;
    }
  ...
  function makeMove(from, to) {
    ...
    const isEnPassantCapture =
      isPawnMove && to === enPassantTarget.value && !targetPiece;
    ...
    if (isCastlingMove(from, to, movingPiece)) {
      ...
      if (isPawnMove || isCapture) moveCountWithoutAction.value = 0;
      else moveCountWithoutAction.value += 1;
    ...
    if (
      isPawnMove &&
      Math.abs(parseSquare(to).rank - parseSquare(from).rank) === 2
    ) {
  ...
  function isCastlingMove(from, to, piece) {
    ...
    if (color === "w") {
      ...
      if (to === "c1") {
        ...
        if (getPieceAt("d1") || getPieceAt("c1") || getPieceAt("b1"))
          return false;
    ...
    if (color === "b") {
      ...
      if (to === "c8") {
        ...
        if (getPieceAt("d8") || getPieceAt("c8") || getPieceAt("b8"))
          return false;
  ...
  function isValidMove(from, to, piece) {
    ...
    if (piece[1] !== "K") {
      const kingSquare = Object.keys(tempBoard).find(
        (sq) => tempBoard[sq] === `${piece[0]}K`
      );
      if (
        isSquareAttacked(kingSquare, piece[0] === "w" ? "b" : "w", tempBoard)
      ) {
